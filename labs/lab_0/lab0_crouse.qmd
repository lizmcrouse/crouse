...
car_data <- read_csv("C:/Users/lizmc/Desktop/Y2S2/ppa/crouse/labs/lab_0/data/car_sales_data.csv")
...
- Rows: 50,000
- Columns: 7 
- Variable types: columns 1,2,4 character; columns 3,5-7 numeric
- Problematic names: Year of manufacture, fuel type, engine size -- R does not like spaces
...
**Your answer:** The tibble provides a cleaner, more efficient processing of the data. Tibble shows a preview of the first 6 rows, and DF shows first 10 with multiple pages to view more.
...
selection1 <- select(car_data, "Model","Mileage")

# Select Manufacturer, Price, and Fuel type
selection2 <- select(car_data,"Manufacturer","Price","Fuel type")

# Challenge: Select all columns EXCEPT Engine Size
selection3 <- select(car_data, "Manufacturer","Price","Fuel type","Model","Mileage")
```
...
**Your answer:** Year of manufacture has spaces which causes issues in R and need to be surrounded by `backticks`, but the simplified "year" name is easily legible.
...
check <- select(car_data, Model, year, age, Mileage, mileage_per_year)
```
...
price_category <- select(car_data, price_category)
...
lowmiles <- filter(car_data, Mileage < 30000)

# Find luxury cars (from price category) with low mileage
low_luxury <- filter(car_data, price_category == "highend")
low_luxury <- filter(low_luxury, Mileage < 30000)
...
midprice <- filter(car_data, Price > 20000 &
                   Price < 35000)

# Find diesel cars less than 10 years old

youngdiesel <- filter(car_data, `Fuel type` == "Diesel", 
                        age < 10)
...
cars_by_manufacturer <- car_data %>%
  count(Manufacturer, sort = TRUE, name = "cars_per_manufacturer")
...
pricecategory_freq <- car_data %>%
  count(price_category, sort = TRUE, name = "cars_per_pricecategory")
```
