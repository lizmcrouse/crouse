...
##For k-nearest neighbor, we chose to evaluate the distance of each property to the nearest 1, 5, and 10 tracts with high heat vulnerability, as this combination of demographic, economic, and physical factors often correlates with property values. 

heat_vulnerability <- st_read("C:/Users/Liz/Desktop/y2s2/PPA/crouse/labs/lab_3/data/heat_vulnerability_ct.geojson")

##Filtering only high heat vulnerability districts
high_heat_vuln <- heat_vulnerability %>%
  filter(n_veryhigh == 1)

##Creating the distance matrix
dist_matrix <- st_distance(phl_props_filtered, high_heat_vuln)
dist_matrix <- as.matrix(dist_matrix)     
dist_matrix <- units::drop_units(dist_matrix)

##Performing the k nearest neighbor function
knn_distance <- function(dist_matrix, k) {
  apply(dist_matrix, 1, function(x) {
    mean(sort(x)[1:k])
  })
}

##Averaging distance to 1, 5, 10 nearest neighbors
avg_knn_1 <- knn_distance(dist_matrix, k = 1)
avg_knn_5 <- knn_distance(dist_matrix, k = 5)
avg_knn_10 <- knn_distance(dist_matrix, k = 10)

##Joining back to properties
phl_props_filtered$avg_knn_1 <- avg_knn_1
phl_props_filtered$avg_knn_5 <- avg_knn_5
phl_props_filtered$avg_knn_10 <- avg_knn_10

##Evaluating results
phl_props_filtered %>%
  st_drop_geometry() %>%
  dplyr::select(avg_knn_5) %>%
  summary()

phl_props_filtered %>%
  st_drop_geometry() %>%
  dplyr::select(avg_knn_1) %>%
  summary()

phl_props_filtered %>%
  st_drop_geometry() %>%
  dplyr::select(avg_knn_10) %>%
  summary()
