...
rehab_phl <- st_read("C:/Users/lizmc/Desktop/Y2S2/ppa/crouse/labs/lab_2/data/DOH_DrugAlcoholTreatmentFacilities202212")
rehab_phl <- rehab_phl %>% st_transform(2272)

mhc_phl <- st_read("C:/Users/lizmc/Desktop/Y2S2/ppa/crouse/labs/lab_2/data/DOH_CommunityMentalHealthCenters202106.geojson")
mhc_phl <- mhc_phl %>% st_transform(2272)

phl <- counties(state = "PA") 
phl <- phl %>%
  filter(NAMELSAD == "Philadelphia County")
phl <- phl %>% st_transform(2272)
pa_tracts <- pa_tracts %>% st_transform(2272)
phl_tracts <- phl_tracts %>% st_transform(2272)
...
Do high-crime areas have equitable access to health services and alternative interventions?
...
phl_tracts <- st_filter(pa_tracts, phl)

crime_by_tract <- st_join(phl_tracts, crime_phl)
