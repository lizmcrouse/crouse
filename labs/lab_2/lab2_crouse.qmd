...
ggplot(county_statistics_summary) + 
  geom_sf(aes(fill = underserved_pct), color = NA) +
  geom_sf(data = pa_counties, fill = NA, color = "black", size = 0.4) +
...
- Are there any patterns in where underserved counties are located? **The largest concentration of the most severely underserved tracts is in north central PA, a rural area distant from PA's two major population centers.**
...
ggplot(county_statistics_summary) + 
  geom_sf(aes(fill = n_unders_tracts), color = NA) +
  geom_sf(
    data = pa_counties,
    fill = NA,
    color = "black",
  ...
  scale_fill_viridis_c(
    option = "mako",
    direction = -1,
    name = "Number of Underserved Tracts"
  ) +
  theme_minimal()
